...
function ra_export_wp($author='', $category='', $post_type='', $status='', $start_date='', $end_date='', $terms = '') {
	...
	if ( $post_type == 'post_attachment') {
		$where = $wpdb->prepare("WHERE post_type='attachment' and post_parent in (select id from wp_posts where post_type='post')");
	} elseif ( $post_type and $post_type != 'all' ) {
...
<option value="post_attachment"><?php _e('post_attachment'); ?></option>
</select>
</td>
</tr>
<tr>
