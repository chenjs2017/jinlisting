	...
	if (PFASSIssetControl('st8_npsys','',0) == 1) {
		$pf_extra_taxonomyfields[] = array(
			...
			'fields' => array(	
				...
				array(
					'name' => esc_html__('改为Tag','pointfindert2d'),
					'id'   => 'change_to_tag',
					'type' => 'radio',
					'options' => array(
						'0' => esc_html__('不变','pointfindert2d'),
						'1' => esc_html__('改为Tag','pointfindert2d'),
						),
					'std'  => 0,
				),
					array(
					'name' => esc_html__('合并子分类','pointfindert2d'),
					'id'   => 'merge_sub_listing',
					'type' => 'radio',
					'options' => array(
						'0' => esc_html__('不变','pointfindert2d'),
		...
		$pf_extra_taxonomyfields[] = array(
			...
			'fields' => array(	
				...
				array(
					...
					'desc' => sprintf(esc_html__("Custom sidebar for only this category.",'pointfindert2d'),'<br/>'),
					'id'   => 'pflt_sidebar',
					'type' => 'select',
