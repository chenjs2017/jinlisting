...
$.maindivfav.children('#itempage-pffav-text').html(obj.favtext);};};};},complete:function(){$.maindivfav.parent('.pflist-item').pfLoadingOverlay({action:'hide'});},});});});$(function(){$('.pf-report-link').live('click',function(){$.maindivfav=$(this);$.ajax({type:'POST',dataType:'json',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_reportitem','item':$.maindivfav.attr('data-pf-num'),'security':theme_scriptspf.pfget_reportitem},success:function(data){var obj=[];$.each(data,function(index,element){obj[index]=element;});if(!$.isEmptyObject(obj)){if(obj.user==0&&obj.rs=="1"){$.pfOpenLogin('open','login');}else if(obj.user==0&&obj.rs=="0"){$.pfOpenModal('open','reportform','','',obj.item);}else if(obj.user!=0&&obj.rs=="1"){$.pfOpenModal('open','reportform','','',obj.item);}else if(obj.user!=0&&obj.rs=="0"){$.pfOpenModal('open','reportform','','',obj.item);};};}});});});$(function(){$('#pfclaimitem').live('click touchstart',function(){$.maindivfav=$(this);$.ajax({type:'POST',dataType:'json',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_claimitem','item':$.maindivfav.attr('data-pf-num'),'security':theme_scriptspf.pfget_claimitem},success:function(data){var obj=[];$.each(data,function(index,element){obj[index]=element;});if(!$.isEmptyObject(obj)){if(obj.user==0&&obj.rs=="1"){$.pfOpenLogin('open','login');}else if(obj.user==0&&obj.rs=="0"){$.pfOpenModal('open','claimform','','',obj.item);}else if(obj.user!=0&&obj.rs=="1"){$.pfOpenModal('open','claimform','','',obj.item);}else if(obj.user!=0&&obj.rs=="0"){$.pfOpenModal('open','claimform','','',obj.item);};};}});});});$.pfReviewwithAjax=function(vars){$.ajax({beforeSend:function(){$('#pftrwcontainer').pfLoadingOverlay({action:'show'});},type:'POST',dataType:'json',url:theme_scriptspf.ajaxurl,data:{'action':'pfget_modalsystemhandler','formtype':'reviewform','vars':vars,'security':theme_scriptspf.pfget_modalsystemhandler},success:function(data){
