...
function pointfinder_extrafunction_03_new(){
    ...
    register_widget( 'pf_featured_agents_w_new' );
		register_widget( 'pf_simple_search' );
};
...
// Creating the widget 
class pf_simple_search extends WP_Widget {
...
__('PF Simple Search', 'wpb_widget_domain'), 
...
**/
    ...
    class pf_search_items_w_new extends WP_Widget {
        ...
        public function widget( $args, $instance ) {
            ...
            if ($front_post_id=='54497')
            {
                $front_page=True;
            }
            if (($if_mobile==True) && ($front_page==False)) {
            }
            else {
            echo $args['before_widget'];
            if ( ! empty( $title ) ){
            ...
            if(!wp_script_is('pfsearch-select2-js', 'enqueued')){wp_enqueue_script('pfsearch-select2-js');}    
              ...
              **/
                ...
                ?>
                <form id="pointfinder-search-form-manual" method="get" action="<?php echo esc_url(home_url()); ?>"
                      data-ajax="false">
