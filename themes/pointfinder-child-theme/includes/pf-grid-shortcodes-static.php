		...
		if(is_array($pfgetdata)){
			...
			if($pfg_orderby != ''){
				if($pfg_orderby == 'date' || $pfg_orderby == 'distance' || $pfg_orderby =='recommend' || $pfg_orderby =='relevant'){
					...
					if ($pfg_orderby =='reviewcount' || $pfg_orderby =='reviewnumber') {
						$pfg_order = 'desc';
			...
			/* Start: Header Area for filters (HTML) */		
            	if($pfheaderfilters == ''){
								...
								if ($review_system_statuscheck == 1) {
									...
									$pfgform_values3_texts['reviewcount'] = esc_html__('评分高至低','pointfindert2d');
									array_push($pfgform_values3, 'reviewnumber');
									$pfgform_values3_texts['reviewnumber'] = esc_html__('评论多至少','pointfindert2d');
								}
								...
								foreach($pfgform_values3 as $pfgform_value3){
										if(isset($pfg_orderby)){
										 if(strcmp($pfgform_value3, $pfg_orderby) == 0){
											 ...
											 $wpflistdata .= '<strong><a href="#" onclick="orderby(\'' .$pfgform_value3. '\')">'.$pfgform_values3_texts[$pfgform_value3].'▶</a></strong>&nbsp;&nbsp;&nbsp;&nbsp;';
										 }
																...
																/* Start: Review Stars */
											                        if ($review_system_statuscheck == 1) {
											                        	if ($setup22_searchresults_hide_re == 0) {
																									...
																									$reviews = get_post_meta( $pfitemid, 'webbupointfinder_item_reviewcount', true );
